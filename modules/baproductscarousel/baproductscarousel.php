...
class BaProductsCarousel extends Module
{
    ...
    public function install()
    {
        ...
        $query .="title varchar(10) not null,cate text not null,sizeslide varchar(255) not null,";
        ...
        foreach ($id_shop as $i) {
            ...
            $sqladd .="VALUES('".pSQL($names)."','0','0','0',";
    ...
    public function hookdisplayHomeTab()
    {
        ...
        $this->context->smarty->assign('num_items', $num_items);
    ...
    public function fixjs()
    {
        ...
        $this->context->smarty->assign('baproductscarousel_base', $base);
    ...
    public function hookDisplayHeader()
    {
        ...
        $html .= '<script>
                 var id_customer_ba = \'' .$id_customer. '\';
                 var rtl = \'' .$rtl. '\';
                </script>';
    ...
    public function addAwesomeFont()
    {
        ...
        $html .= $this->display(__FILE__, 'views/templates/front/awesome_font.tpl');
        return $html;
    }
    public function templateslide($checktype, $id_sli = null)
    {
        ...
        $this->context->smarty->assign('baproductscarousel_token', $token);
        ...
        if (isset($test)) {
            foreach ($test as $key) {
                ...
                $title = $key['title'];
                $names = json_decode($key['name'], true);
                $names = $names[$id_lang];
                ...
                if ($key['ordertype'] == 'bestsell') {
                    ...
                    if (is_array($pros)) {
                        foreach ($pros as $pross) {
                            ...
                            if ($ss->active == 1) {
                                $taxes = Product::getTaxesInformations($id_fix);
                                $arrs[$name_p] = array(
                                    'name' => $ss->name,
                ...
                $this->context->smarty->assign('id_customer_ba', $id_customer);
    ...
    public function addItem()
    {
        ...
        if (Tools::isSubmit('add_item')) {
            ...
            $name_item = json_encode(Tools::getValue('name_item'));
            ...
            // neu chua cai thi stop
            $wishlist_path = file_exists(_PS_MODULE_DIR_.'blockwishlist/blockwishlist.php');
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue('updatebaproductscarousel') !== false && Tools::getValue('id')) {
            ...
            $showids = $db->getRow($sqlshow);
            $sqltimg = "SELECT * FROM "._DB_PREFIX_."image_type";
            $showtimg = $db->ExecuteS($sqltimg);
        
            $json = str_replace("['[',']']", "", json_decode($showids['cate']));
            $prods = str_replace("['[',']']", "", json_decode($showids['active_pro']));
            $showids['name'] = json_decode($showids['name'], true);
            if (is_array($json)) {
            ...
            $iso_l = Language::getIsoById($id_default_l);
            $this->smarty->assign('id_default_language', $id_default_l);
            $this->smarty->assign('iso_default_language', $iso_l);
