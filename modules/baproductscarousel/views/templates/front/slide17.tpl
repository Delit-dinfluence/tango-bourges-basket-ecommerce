							...
							{foreach from=$im item=item2}
								<div style="" class="item slier_item">
									<div class="js-product-miniature" data-id-product="{$item.id_product|escape:'htmlall':'UTF-8'}" data-id-product-attribute="{$item.cache_default_attribute|escape:'htmlall':'UTF-8'}">
										...
										{/if}
								    	<a class='quiss quick-view' rel="{BaProductsCarousel::getUrlFix($item.id_product|escape:'htmlall':'UTF-8')}" title="" data-link-action="quickview"><i class="fa fa-eye"></i></a>
										<div class="ad_info_pro">
											<h4><a href="{BaProductsCarousel::getUrlFix($item.id_product|escape:'htmlall':'UTF-8')}">{$item.name|escape:'htmlall':'UTF-8'}</a></h4>
											{if $price == 1}
											    <span style="font-size: 1.1rem" class="price_pro">{Tools::displayPrice($lb4|escape:'htmlall':'UTF-8')}</span>
											    {if $lb3 !== $lb4 && $item.price>0}
											...
											{if $addtocart->addcart == 1}
												<div class="add_to_carsou button-container" >
													<form action="{$urls.pages.cart|escape:'htmlall':'UTF-8'}" method="post" accept-charset="utf-8">
														...
														<a  style="background: #C4C4C4 !important" class="ajax_add_to_cart_button add-to-cart" {if Product::getQuantity($item.id_product) > 0} data-button-action="add-to-cart"  rel="nofollow" title="<a title="{l s='Add to cart' mod='baproductscarousel'}"  data-id-product="{$item.id_product|escape:'htmlall':'UTF-8'}"{/if}>
														<span>{l s='Out of stock' mod='baproductscarousel'}</span>
														</a>
														{else}
														<button type="submit" class="ajax_add_to_cart_button add-to-cart" {if Product::getQuantity($item.id_product) > 0} data-button-action="add-to-cart"  rel="nofollow" title="{l s='Add to cart' mod='baproductscarousel'}"  data-id-product="{$item.id_product|escape:'htmlall':'UTF-8'}"{/if}>
