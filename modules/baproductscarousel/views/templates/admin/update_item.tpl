			...
			<div {if $cbt != null} style="display: none;" {/if} class="panel general-panel">
				...
				<div style="margin-top: 15px" class="row">
					...
					<div class="col-lg-7">
						...
						<div class="col-lg-6 translatable-field row lang-{$language.id_lang|escape:'htmlall':'UTF-8'}" style="{if $id_default_language == $language.id_lang}display:block;{/if} {if $id_default_language != $language.id_lang}display:none;{/if}" >
							<div>
								<input type="text" id="name_item_{$language.id_lang|escape:'htmlall':'UTF-8'}" class="form-control updateCurrentText " name="name_item[{$language.id_lang|escape:'htmlall':'UTF-8'}]" value="{$item.name.$id_l|escape:'htmlall':'UTF-8'}"/>
							</div>
						</div>
						{/foreach}
						<div class="col-lg-1">
							...
							<ul class="dropdown-menu">
								{foreach $languages item=language}
									<li>
										<a  href="javascript:void(0);" onclick="baproductscarousel_hideOtherLanguage('{$language.id_lang|escape:'htmlall':'UTF-8'}','{$language.iso_code|escape:'htmlall':'UTF-8'}')">{$language.iso_code|escape:'htmlall':'UTF-8'}</a>
									</li>
								{/foreach}
							</ul>
						</div>
					...
					<div class="col-lg-8">
						...
						<div class="add">
							{if $product[0] !== ''}
									...
									<script>
										var url = "{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/{$prods->category|escape:'htmlall':'UTF-8'}/{$prods->id|escape:'htmlall':'UTF-8'}-{strtolower(str_replace(' ','-',$prods->name|escape:'htmlall':'UTF-8'))}"+".html";
									</script>
										<div id="div{$prods->id|escape:'htmlall':'UTF-8'}" style="margin-top:10px;float:left;width:100%">
											...
											<a class="urlss" target="_blank" href="{$baproductscarousel_base|escape:'htmlall':'UTF-8'}{$iso_lang|escape:'htmlall':'UTF-8'}/{$prods->category|escape:'htmlall':'UTF-8'}/{$prods->id|escape:'htmlall':'UTF-8'}-{$prods->link_rewrite|escape:'htmlall':'UTF-8'}.html" style="float:left;font-size:12px;margin-top: 3px;color:black;">{$prods->name|escape:'htmlall':'UTF-8'} (id: {$prods->id|escape:'htmlall':'UTF-8'})</a>
										</div>
									{/foreach}
								{else}
